...
const getCandidateRows = (stateName, elections, handleWinnerSelect) => {
    ...
    return candidates.map((candidate, i) => {
        return election.projectedWinner && candidate.name === election.projectedWinner.name ? 
            <td key={i} style={{background: Helpers.getPartyColor(candidate.party), textShadow: "black 1px 1px 3px", color: "white"}}><Octicon icon={Octicons.check}/> {getCandidateFormat(candidate, i)}</td>
            : 
